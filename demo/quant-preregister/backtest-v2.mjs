...
// ── 0. 自校验：预注册没被动过，才允许跑 ──────────────────────────────────────
const md = fs.readFileSync(path.join(HERE, 'PREREGISTRATION-v2.md'), 'utf8');
const promised = md.match(/sha256 = ([0-9a-f]{64})/)?.[1] ?? null;
const actual = observe(`${REL}/prereg-v2.json`, REPO).properties.sha256;
...
const P = JSON.parse(fs.readFileSync(path.join(HERE, 'prereg-v2.json'), 'utf8'));
...
// ── 2. 策略与指标 ────────────────────────────────────────────────────────────
// v2：滑动窗口 O(n) + 按种子缓存各窗口 —— 纯实现优化，语义不变，由 §4 的复算判据背书。
const sma = (px, w) => {
  let s = 0; const out = new Array(px.length);
  for (let i = 0; i < px.length; i++) {
    s += px[i]; if (i >= w) s -= px[i - w];
    out[i] = i >= w - 1 ? s / w : null;
  }
  return out;
};
let MA_CACHE = null;   // 每个种子换一份
const ma = (px, w) => (MA_CACHE[w] ||= sma(px, w));
...
function dailyPnl(px, fast, slow) {
  const f = ma(px, fast), s = ma(px, slow);
...
// §4 语义不变性复算：QUANT_GRID=v1 时用第一轮的网格跑，结果须与 RESULTS.json 逐位相同。
// 优化必须被验证而不是被声称——这与「修好了就说修好了」是同一条规矩。
const RECHECK = process.env.QUANT_GRID === 'v1';
const GRID = RECHECK ? JSON.parse(fs.readFileSync(path.join(HERE, 'prereg.json'), 'utf8')).grid : P.grid;
const grid = [];
for (const fast of GRID.fast) for (const slow of GRID.slow) if (fast < slow) grid.push({ fast, slow });
if (grid.length !== GRID.n_trials) {
  console.error(`拒跑：网格实际 ${grid.length} 组，预注册写的是 ${GRID.n_trials}`);
  process.exit(3);
}
...
fs.writeFileSync(path.join(HERE, RECHECK ? 'RESULTS-v2-recheck.json' : 'RESULTS-v2.json'), JSON.stringify(out, null, 2) + '\n');
...
console.log('');
console.log('→ demo/quant-preregister/RESULTS-v2.json');
