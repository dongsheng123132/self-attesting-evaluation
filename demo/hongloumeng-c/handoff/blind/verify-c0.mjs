...
for (const p of spec.predicates.filter(x => x.kind === 'order' || x.kind === 'configuration')) {
  const legs = p.legs_calibrated_by || [];
  if (!legs.length) { problems.push(`${p.id}（${p.kind} 复合谓词）未声明 legs_calibrated_by —— 它的各条腿有没有被单独校准无从得知`); continue; }
  const missing = legs.filter(id => !covered.has(id));
  if (missing.length) problems.push(`${p.id} 声明由 ${missing.join('、')} 校准各腿，但这些 id 不在校准集里`);
}
